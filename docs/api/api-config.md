...
### `Config.saves.isEvalEnabled` ↔ `boolean` (default: `true`) {#config-api-property-saves-isevalenabled}

Determines whether the evaluation of JavaScript code generated by `Serial.createReviver()` is enabled when loading saves.

<p role="note" class="warning"><b>Warning:</b>
Changing this setting's value to false will invalidate all saves and sessions generated if you used `Serial.createReviver()` for your save games.
</p>

<p role="note" class="warning"><b>Warning:</b>
Enabling this setting can lead to security vulnerabilities if untrusted parties are able to modify save data.  It is <strong><em>strongly recommended</em></strong> to disable this setting if you plan on loading save files from your player base, e.g. for debugging purposes.
</p>
...
A `boolean` value signifying whether the evaluation of JavaScript code generated by `Serial.createReviver()` is enabled.

#### Examples:

```javascript
Config.saves.isEvalEnabled = false;
```
